...
std::function<void(Scene*& s)> scene_example_init = [](Scene*& s){
	...
	r.emplace<c_vel>(entity, glm::vec2(0.31, 0.69));
	r.emplace<c_rend>(entity, manager.textures["z_d1"], glm::vec2(1, 1), glm::vec2(1, 1));
...
std::function<void(Scene*&)> scene_example_update = [](Scene*& s){
	...
	for(auto& e : rend_view){
		...
		Rectangle dst = (Rectangle){
			(p.pos.x - s->cam.pos.x) * METER * RATIO,
			(p.pos.y - s->cam.pos.y) * METER * RATIO,
